...
Builds the vendored MLX MelBand-RoFormer from the package's own config and its own
...
control. Mixed precision is forced off because bs-roformer-infer measured it to
buy no speed on the equivalent BS-RoFormer path while degrading Torch-vs-MLX
agreement by roughly 87x, and article 2 requires an accuracy-affecting
optimization to be opt-in; the same env var gates both packages' MLX models.

Two refusals are deliberate. A checkpoint whose registry `variation` names a
mask-estimator head this backend does not build is rejected rather than run.
melband-roformer-infer's registry currently declares no variations at all (see
`test_every_registry_variation_has_an_mlx_head`), so today this refuses anything
other than the stock head -- the hook exists so a future variant checkpoint fails
loudly at selection time instead of constructing silently as the wrong
architecture, the exact failure bs-roformer-infer's four variant checkpoints hit
before their MLX heads were written. And a config whose chunk_size is not a
multiple of its STFT hop is rejected, because the chunked path's length
bookkeeping silently assumes that alignment.

Reads: .base (ChunkingPlan, BackendUnavailable), ..mlx (MelBandRoformerMLX,
conversion), ..utils (load_checkpoint_state), numpy
"""
...
#: Variations this backend can build. melband-roformer-infer's registry declares
#: none today (no MelBand checkpoint here carries a `variation` field), so this is
#: intentionally just the stock sentinels -- extend it the day a variant head is
#: ported, the same way bs-roformer-infer's `mlx/heads/` measures its own set.
_SUPPORTED_VARIATIONS = frozenset({None, "", "mlp"})


def supported_variations() -> frozenset:
    return _SUPPORTED_VARIATIONS


class MLXBackend:
    """Runs MelBand-RoFormer natively on Apple Silicon through MLX."""
    ...
    def _require(cls):
        if not cls.is_available():
            raise BackendUnavailable(
                ...
                "pip install 'melband-roformer-infer[mlx]' (Apple Silicon)"
            )
    ...
    def from_checkpoint(cls, *, config, checkpoint_path, variation=None, device="mps"):
        ...
        model = MelBandRoformerMLX(**cls._model_args(config))
    ...
    def _model_args(config) -> dict:
        ...
        args = {
            ...
            "match_input_audio_length": model_cfg.get("match_input_audio_length", False),
        }
