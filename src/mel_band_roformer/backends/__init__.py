...
def resolve_backend_name(requested: str | None, *, variation: str | None = None) -> str:
    ...
    (melband-roformer-infer's registry currently declares none, but the hook is
    kept so a future variant checkpoint fails the same way bs-roformer-infer's
    did rather than being newly invented here). Passing it matters: without it,
    `auto` can settle on a backend that has no head for the model and then fail
    at construction, where Torch would simply have worked. An *explicit* backend
    still raises for a variation it cannot run -- that request is honoured or
    ...
    if not backend.is_available():
        raise BackendUnavailable(
            ...
            f"it may need an optional extra (pip install 'melband-roformer-infer[{requested}]')"
        )
